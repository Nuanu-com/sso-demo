...
const islands = {
  "session-status": () => import("./session-status.jsx"),
};
...
export function mountIslands(root = document) {
  root.querySelectorAll("[data-island]").forEach(async (el) => {
    ...
    // this node - including replacing the static version Go rendered there.
    // Anything the island renders that uses basecoat markup gets picked up by
    // basecoat's MutationObserver automatically.
